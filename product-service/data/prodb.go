...
func (l *Product) Get(maxPrice float64, minPrice float64) ([]*Product, error) {
	...
	if minPrice != 0 || maxPrice != 0 {
		priceFilter := bson.D{}
		if minPrice != 0 {
			priceFilter = append(priceFilter, bson.E{"price", bson.D{{"$gte", minPrice}}})
		}
		if maxPrice != 0 {
			priceFilter = append(priceFilter, bson.E{"price", bson.D{{"$lte", maxPrice}}})
		}
		filter = append(filter, bson.E{"$and", priceFilter})
	}
